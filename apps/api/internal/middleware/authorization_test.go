...
type stubAuthorizationService struct {
	...
	object  service.AuthorizationObject
	action  string
}

func (s *stubAuthorizationService) Enforce(_ context.Context, sub service.AuthorizationSubject, obj service.AuthorizationObject, act string) (bool, error) {
	s.called = true
	s.subject = sub
	s.object = obj
	s.action = act
